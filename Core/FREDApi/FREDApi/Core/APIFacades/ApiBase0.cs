...
namespace FRED.Api.Core.ApiFacades
{
	...
	public abstract class ApiBase0<TArguments, TContainer> : IApi<TArguments, TContainer>
		...
		where TContainer : class
	{
		...
		public ApiBase0(IRequest request)
		{
		...
		public virtual string Fetch()
		{
            ...
            try
			{
				...
				Request.Json = Json;
				container = Request.Fetch(Arguments);
				SetResultProperties();
			}
		...
		public async Task<string> FetchAsync()
		{
			...
			try
			{
				...
				//Request.Deserialize = typeof(TContainer) != typeof(string);
				Request.Json = Json;
				container = await Request.FetchAsync(Arguments);
				SetResultProperties();
			}
			catch (Exception exception)
			{
